...
resource "payment_gateway/purchases", document: :v1 do
  ...
  let!(:card) { c = create(:card, customer: customer, token: '40C8D89053570AE5E053321E320A7559'); customer.cards << c; c }
  ...
  post "/purchases" do
    # NOTE: vcr cassete was recorded on the 2nd card payment
    include SpecHelpers::VcrDoRequest[:payment_gateway_purchases_3d_secure_card]
    ...
    context 'Validation errors all nils' do
      let(:amount) { nil }
      let(:card_token) { nil }
      let(:order_id) { nil }
      let(:card_verification_value) { 123 }
      example_request 'Validation errors: all nils' do
    ...
    context 'Validation errors all blank' do
      let(:amount) { '' }
      let(:card_token) { '' }
      let(:order_id) { '' }
      let(:card_verification_value) { nil }
      example_request 'Validation errors: all blank' do
        expect(status).to eq(422)
        # ap json(response_body)
        expect(response_headers['Content-Type']).to match("#{Mime[:json].to_s}; charset=utf-8")
      end
