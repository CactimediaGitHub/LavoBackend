...
resource "order/cart_total", document: :v1 do
  ...
  let(:order_items) { [
    ...
    { :'inventory-item-id' => inventory_item1.id, quantity: 2 }
  ] }
  ...
  let!(:promotion) { create(:promotion_with_order_total_flat_rate_on_order_total, vendors: [vendor]) }

  post "/cart" do
    context 'Flate rate promotion discount given on order total if order total is in specified range' do
      example_request 'Flate rate promotion discount given on order total if order total is in specified range' do
        expect(status).to eq(200)
        ap json(response_body)

        expect(response_headers['Content-Type']).to match("#{Mime[:json].to_s}; charset=utf-8")
        expect(json(response_body)[:data][:attributes][:subtotal]).to eq(5000)
        expect(json(response_body)[:data][:attributes][:total]).to eq(6350)
